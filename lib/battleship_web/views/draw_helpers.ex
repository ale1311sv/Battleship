defmodule BattleshipWeb.DrawHelpers do
  ...
  def clickable(cell, boats, boat_selected, first_cell_selected) do
    cond do
      ...
      is_nil(first_cell_selected) -> "enabled"
      true ->
        if Operations.is_second_cell?(boat_selected, first_cell_selected, boats, cell),
  ...
  defp content({row, column}, boats) do
    ...
    cond do
      main_cell == :water -> "water"

      # Is it surrounded by 2 boat cells? It must be the BODY
      Enum.count([left_cell, right_cell, top_cell, bottom_cell], &(&1 == :boat)) == 2 -> "boat_body"
      ...
      (top_cell == :water and bottom_cell == :boat) or
          (left_cell == :water and right_cell == :boat) -> "boat_tail"
